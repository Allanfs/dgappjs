...
module.exports =  function(app, db){
  ...
  app.post('/clientes', async (req, res) => {
    ...
    let dadosTelefone = [b.telefone.ddd, b.telefone.numero, b.telefone.whatsapp, b.telefone.observacao]
    let dadosEndereco = [b.endereco.rua, b.endereco.bairro, b.endereco.complemento, b.endereco.numero]
  ...
  try {
      ...
      // INSERIR CLIENTE
      const idCliente = await db.query(qInsertCliente, dadosCliente).then(r=> r.rows[0].id_cliente)
  ...
  app.delete('/clientes/:id', async (req, res) => {
    ...
    const existe = await db.query(`select true from dg.tb_cliente where id_cliente = $1`, param)
    .then(r=> {
        if (r.rowCount === 0) {
          return false
        }
...
const qInsertTelefone = {
  ...
  text: sqlgen.insert("dg.tb_telefone",{ddd: "$1", numero:"$2", whatsapp:"$3", observacao: "$4", id_cliente:"$5"}).sql
}

const qInsertCliente = {
  name: 'insert-cliente',
  text: sqlgen.insert("dg.tb_cliente",{nome: "$1", cpf:"$2",instagram:"$3", email:"$4", data_nascimento: "$5::DATE"}).sql
}

const qInsertEndereco = {
  name: 'insert-endereco',
  text: sqlgen.insert("dg.tb_endereco",{rua: "$1", bairro:"$2",complemento:"$3", numero:"$4", id_cliente: "$5"}).sql
}

const qUpdateCliente = {
  text: sqlgen.update("dg.tb_endereco",{id_cliente: "$1"},{rua: "$1", bairro:"$2",complemento:"$3", numero:"$4", id_cliente: "$5"}).sql
}